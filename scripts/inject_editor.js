...
(function() {
	...
	var EXT_ID = "pageedit",
		TEXTAREA_ID = "__CKEditor_textArea__";
	...
	function onMessage(message) {
		...
		function onload() {
			...
			pageedit.editor.setData(pageContent, function() {
				pageedit.editor.addCommand('save', new CKEDITOR.dialogCommand(
					'saveDialog'));
		...
		if (message.toggleEditor) {
			if (pageedit && pageedit.editor) {
				...
				for (i = 0; i < message.scripts.length; i++)
					document.write("<script type='text/javascript' src='" + message.extensionURI +
						message.scripts[i] + "?" + ("" + Math.random()).split(".")[1] +
						"'></script>");
				document.write("</body>");
				document.write("</html>");
				document.close();
	...
	document.head.removeChild(document.head.childNodes[document.head.childNodes.length -
		1]);
